...
$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Shipment Id');
...
$objPHPExcel->getActiveSheet()->setCellValue('G2', 'Shipment Date');
...
if (mysqli_num_rows($result)>0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B' . ($i + 2) , ($sale_id));
...
ob_clean();
$objPHPExcel->getActiveSheet()->setCellValue('A1'," Sale Shipment List");
//$objPHPExcel->getActiveSheet()->setCellValue('A2');
...
$objWriter->save($_SERVER["DOCUMENT_ROOT"].'/sale_shipment.csv');
sleep(1);

header('Content-type: application/octet-stream');
header("Content-Disposition: attachment; filename=sale_shipment.csv");
readfile($_SERVER["DOCUMENT_ROOT"].'/sale_shipment.csv');

@unlink($_SERVER["DOCUMENT_ROOT"].'/sale_shipment.csv');
