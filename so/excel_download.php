...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'sale Order Id');
$objPHPExcel->getActiveSheet()->setCellValue('C2', 'sale Order Date');
...
if($pur_id == "" ) {
    $sql = "SELECT * FROM so WHERE so_date  BETWEEN '$from_date' AND '$to_date'";
}
else {
    $sql = "SELECT * FROM so WHERE so_date  BETWEEN '$from_date' AND '$to_date' $pur_idSql";
}
...
if (mysqli_num_rows($result)>0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        $so_id=$row['so_id'];
        $so_date=$row['so_date'];
        $int_date = date('d-m-Y', strtotime($so_date));
//        $notes=$row['notes'];
//        $added_by=$row['added_by'];
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B' . ($i + 2) , ($so_id));
...
ob_clean();
$objPHPExcel->getActiveSheet()->setCellValue('A1'," sale Order List");
//$objPHPExcel->getActiveSheet()->setCellValue('A2');
...
$objWriter->save($_SERVER["DOCUMENT_ROOT"].'/so.csv');
sleep(1);

header('Content-type: application/octet-stream');
header("Content-Disposition: attachment; filename=so.csv");
readfile($_SERVER["DOCUMENT_ROOT"].'/so.csv');

@unlink($_SERVER["DOCUMENT_ROOT"].'/so.csv');
