...
//        if (mysqli_num_rows($resValidate) > 0 || ($adjustment_id == $old_pa_id)) {
        ...
        $sqlUpdate = "UPDATE `so` SET `so_id`='$so_id',`so_date`='$so_date',`notes`='$notes',`customer` = '$customer_id',`payment_terms`='$payment_terms',
                        `discount` = '$discount',`tds`='$tds',`grand_total` = '$grand_total',`payment_status`='$payment_status',`status` = '$status',`due_date` = '$d_date',`e_way`='$e_way',`po_no` = '$po_no',`po_date`='$po_date',`total_tax`='$totaltax' WHERE `so_id`='$so_id'";
...
//            $discount = str_replace('%', '', $discount);
            ...
            $so_details_id = "SO" . ($ID);
            $sqlUpdates = "UPDATE so_details SET so_details_id = '$so_details_id',so_id='$so_id',customer='$customer_id',so_date='$so_date' WHERE id ='$ID'";
...
//            mysqli_query($conn, $sqlStockUpdate);
        ...
        $info = urlencode("Sale Order Edited");
