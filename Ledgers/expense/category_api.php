...
//    $emp_id = $_COOKIE['emp_id'];
    ...
    if(mysqli_num_rows($resValidateCookie) > 0)
    {
        ...
        $category_sub_name = array();
        $category_sub_id = array();

        $sql="SELECT * FROM expense_category WHERE category_type='$expense_type'";
        $result=mysqli_query($conn,$sql);
        if(mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_array($result)) {

                $category_sub_name[] = $row['category_name'];
                $category_sub_id[] = $row['category_id'];

            }
        }
        $json_array['status'] = 'success';
        $json_array['category_sub'] =  $category_sub_name;
