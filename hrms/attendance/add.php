...
if(isset($_POST['id']) && isset($_POST['type'])&& isset($_POST['date'])&& isset($_POST['hours'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        if($type=="logIn") {
            ...
            $sqlqueryss = "SELECT * FROM attendance WHERE emp_id = '$emp_id' AND login >= '$datee' ORDER BY login DESC ";
...
//            $sqlquerys = "SELECT * FROM attendance WHERE emp_id = '$emp_id' AND date_time = '$date' ORDER BY login DESC";
            $sqlquerys = "SELECT * FROM attendance WHERE emp_id = '$emp_id' AND login >= '$date' ORDER BY login DESC ";
            ...
            if (mysqli_num_rows($results) > 0) {
                ...
                $current_time = time(); // Assuming $current_time is a string representing the current time
