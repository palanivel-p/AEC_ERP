...
<!--                                </div>-->
                                <div class="form-group col-md-4">
                                    ...
                                    <select data-search="true" class="form-control tail-select w-full" id="payment_terms" name="payment_terms"  style="border-color: #181f5a;color: black">
                                        ...
                                        <option value='0' <?php echo ($payment_terms == '0') ? 'selected' : ''; ?>>Immediate</option>
...
//                                            $sql = "SELECT * FROM purchase_details WHERE purchase_id ='$purchase_id'";
                                            $sql = "SELECT  pd.product_id,pd.product_code,
        pd.product_name,pd.Stock_count, pd.qty, pd.unit,pd.unit_cost,pd.discount, pd.dis_symbl, pd.discount_value, pd.discount_type,pd.tax,pd.tax_value,pd.sub_total
    FROM purchase_details pd
    LEFT JOIN  purchase_return_details prd 
    ON  pd.product_id = prd.product_id  WHERE pd.purchase_id = '$purchase_id' AND pd.qty > 0";
...
<script>
    ...
    function editTitle(data) {
        ...
        var edit_model_title = "Return Purchase - "+data;
