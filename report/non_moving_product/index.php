...
<head>
    ...
    <title>Non Moving Product Report</title>
...
<div id="main-wrapper">
    ...
    $header_name ="Non Moving Product Report";
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Non Moving Product Report List</h4>
...
<!--                        <button type="button" class="pdf_download btn btn-primary mb-2" id="pdf" style="margin-left: 20px;">PDF</button>-->
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-responsive-md" style="text-align: center;">
                            ...
                            <tr>
                                ...
                                <th><strong>Date-month</strong></th>
                    ...
                    SELECT notes, s.sale_id, invoice_no, sd.product_id, 
                           MAX(CONVERT(s.sale_date, DATE)) AS maxsaledate, 
                           DATE_FORMAT(MAX(CONVERT(s.sale_date, DATE)), "%d-%m-%Y") AS maxsale_date, 
                           qty, stock_qty, p.product_name 
                    FROM sale s 
                    INNER JOIN sale_details sd ON s.sale_id=sd.sale_id 
                    INNER JOIN product p ON p.product_id=sd.product_id 
                    GROUP BY sd.product_id 
                    HAVING maxsaledate < CONVERT(NOW() - INTERVAL 4 MONTH, DATE)
                ) AS subquery';
...
//                            $sql = 'select notes,s.sale_id,invoice_no,sd.product_id,max(CONVERT(s.sale_date, DATE)) maxsaledate,DATE_FORMAT(max(CONVERT(s.sale_date, DATE)),"%d-%m-%Y") maxsale_date,qty,stock_qty,p.product_name from sale s inner join sale_details sd on s.sale_id=sd.sale_id inner join product p on p.product_id=sd.product_id group by sd.product_id HAVING max(CONVERT(s.sale_date, DATE))<CONVERT(NOW() - INTERVAL 4 MONTH, DATE)';
                            ...
                            $dataSql = 'SELECT notes, s.sale_id, invoice_no, sd.product_id, 
                                   MAX(CONVERT(s.sale_date, DATE)) AS maxsaledate, 
                                   DATE_FORMAT(MAX(CONVERT(s.sale_date, DATE)), "%d-%m-%Y") AS maxsale_date, 
                                   qty, stock_qty, p.product_name 
                            FROM sale s 
