...
$objPHPExcel->getActiveSheet()->setCellValue($startCell, 'Sum Of Qty');

// Add two columns before "Sum Of Amount" for sum of quantity and average rate
$objPHPExcel->getActiveSheet()->insertNewColumnBefore(PHPExcel_Cell::stringFromColumnIndex($columnIndex), 2);

// Set headers for sum of quantity and average rate
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($columnIndex, 2, 'Sum of Amount');
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($columnIndex ,2, 'Average Rate');
...
//                    while ($row1 = mysqli_fetch_assoc($result1)) {
                        ...
                        for ($a = 0; $a < $total_diff_months; $a++) {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($columnIndexes, $columnIndexesa, $totalrate);
...
// Set background color for total amount based on condition
                            $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes, $columnIndexesa)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
                            $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes, $columnIndexesa)->getFill()->getStartColor()->setRGB($totalqty == 0 || $totalqty === null || $totalqty === '' ? 'ff4d4d' : '00b33c');
                            $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes, $columnIndexesa)->getFill()->getStartColor()->setRGB($totalrate == 0 || $totalrate === null || $totalrate === '' ? 'ff4d4d' : '00b33c');
