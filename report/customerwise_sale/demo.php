...
for ($i = 0; $i < $total_diff_months; $i++) {
    ...
    for ($col = $columnIndex; $col <= $columnIndex + 5; $col++) {
...
$startCellQty = PHPExcel_Cell::stringFromColumnIndex($columnIndex) . '2';
$endCellQty = PHPExcel_Cell::stringFromColumnIndex($columnIndex) . '3';
$mergedCellQty = $startCellQty . ':' . $endCellQty;

$objPHPExcel->getActiveSheet()->mergeCells($mergedCellQty);

// Set header value in the top-left cell of the merged range for Sum of Qty
$objPHPExcel->getActiveSheet()->setCellValue($startCellQty, 'Sum Of Qty');
$objPHPExcel->getActiveSheet()->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($columnIndex))->setWidth(20);

$startCellRate = PHPExcel_Cell::stringFromColumnIndex($columnIndex + 1) . '2';
$endCellRate = PHPExcel_Cell::stringFromColumnIndex($columnIndex + 1) . '3';
$mergedCellRate = $startCellRate . ':' . $endCellRate;

$objPHPExcel->getActiveSheet()->mergeCells($mergedCellRate);

// Set header value in the top-left cell of the merged range for Sum of Rate
$objPHPExcel->getActiveSheet()->setCellValue($startCellRate, 'Average Of Rate');
$objPHPExcel->getActiveSheet()->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($columnIndex + 1))->setWidth(20);

$startCellAmount = PHPExcel_Cell::stringFromColumnIndex($columnIndex + 2) . '2';
$endCellAmount = PHPExcel_Cell::stringFromColumnIndex($columnIndex + 2) . '3';
$mergedCellAmount = $startCellAmount . ':' . $endCellAmount;

$objPHPExcel->getActiveSheet()->mergeCells($mergedCellAmount);

// Set header value in the top-left cell of the merged range for Sum of Amount
$objPHPExcel->getActiveSheet()->setCellValue($startCellAmount, 'Sum Of Amount');
$objPHPExcel->getActiveSheet()->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($columnIndex + 2))->setWidth(20);
...
$finalColumnIndex = PHPExcel_Cell::stringFromColumnIndex($columnIndex+2);
...
$sqlCus = "SELECT * FROM `customer` ORDER BY customer_id ASC ";
...
//                $sql1 = "SELECT * FROM `sale_details`,`sale` WHERE sale_details.sale_id=sale.sale_id AND sale_details.product_id='$product_id' AND sale_details.customer='$customerIds' AND sale.sale_date BETWEEN '$fdate1' AND '$ldate1' ORDER BY sale.customer ASC";
                $sql1 = "SELECT * FROM `sale_details`,`sale` WHERE sale_details.sale_id=sale.sale_id AND sale_details.product_id='$product_id' AND sale_details.customer='$customerIds'ORDER BY sale.customer ASC";
...
//                        $sql2 = "SELECT SUM(sale_details.qty) AS quantity,sale_details.unit_cost AS rate FROM `sale_details`,`sale` WHERE sale_details.sale_id=sale.sale_id  AND sale_details.product_id='$product_id' AND sale_details.customer='$customer_id' AND sale.sale_date BETWEEN '$fromD' AND '$toD'";
                        $sql2 = "SELECT SUM(sale_details.qty) AS quantity,sale_details.unit_cost AS rate FROM `sale_details`,`sale` WHERE sale_details.sale_id=sale.sale_id  AND sale_details.product_id='$product_id' AND sale_details.customer='$customer_id'";
                        ...
                        if (mysqli_num_rows($result2) > 0) {
                            ...
                            $bgColorQuantity = $quantity == 0 || $quantity == null || $quantity =='' ? 'f57f7f' : '0bde52';
                            $bgColorRate =  $rate == 0 || $rate == null || $rate =='' ? 'f57f7f' : '0bde52';
                            $bgColorAmt =  $amt == 0 || $amt == null || $amt =='' ? 'f57f7f' : '0bde52';
                        ...
                        $totalrate = ($totalqty != 0) ? $totalAmount / $totalqty : 0;

                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($columnIndexes, $columnIndexesa, $totalqty);
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($columnIndexes + 1, $columnIndexesa, $totalrate);
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($columnIndexes + 2, $columnIndexesa, $totalAmount);

                        $fillColor = ($totalqty == 0 || $totalqty === null || $totalqty === '') ? 'f57f7f' : '0bde52';
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes, $columnIndexesa)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes, $columnIndexesa)->getFill()->getStartColor()->setRGB($fillColor);

                        $fillColor = ($totalrate == 0 || $totalrate === null || $totalrate === '') ? 'f57f7f' : '0bde52';
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes + 1, $columnIndexesa)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes + 1, $columnIndexesa)->getFill()->getStartColor()->setRGB($fillColor);

                        $fillColor = ($totalAmount == 0 || $totalAmount === null || $totalAmount === '') ? 'f57f7f' : '0bde52';
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes + 2, $columnIndexesa)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($columnIndexes + 2, $columnIndexesa)->getFill()->getStartColor()->setRGB($fillColor);
...
// Add totals row
$i++;
// Set the value "Total" in merged cells A to C of the new row
$objPHPExcel->getActiveSheet()->mergeCells('A' . ($i + 3) . ':C' . ($i + 3));
$objPHPExcel->getActiveSheet()->setCellValue('A' . ($i + 3), 'Total');
$objPHPExcel->getActiveSheet()->getStyle('A' . ($i + 3))->getFont()->setSize(18);
$objPHPExcel->getActiveSheet()->getStyle('A' . ($i + 3))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

// Iterate over each column index
for ($colIndex = 3; $colIndex <= $columnIndexes; $colIndex += 3) {
    // Calculate total quantity, rate, and amount for the column
    $totalQty = 0;
    $totalRate = 0;
    $totalAmt = 0;

    // Iterate over each row within the column
    for ($rowIndex = 4; $rowIndex < $columnIndexesa; $rowIndex++) {
        // Accumulate values
        $totalQty += $objPHPExcel->getActiveSheet()->getCellByColumnAndRow($colIndex, $rowIndex)->getValue();
        $totalRate += $objPHPExcel->getActiveSheet()->getCellByColumnAndRow($colIndex + 1, $rowIndex)->getValue();
        $totalAmt += $objPHPExcel->getActiveSheet()->getCellByColumnAndRow($colIndex + 2, $rowIndex)->getValue();
    }

    // Set total values in the corresponding cells
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($colIndex, $i + 3, $totalQty);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($colIndex + 1, $i + 3, $totalRate);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($colIndex + 2, $i + 3, $totalAmt);
}
...
// Merge cells from column A to column C in the total row
$objPHPExcel->getActiveSheet()->mergeCells('A' . ($i + 3) . ':C' . ($i + 3));

// Apply background color to the merged cells
$objPHPExcel->getActiveSheet()->getStyle('A' . ($i + 3) . ':C' . ($i + 3))->applyFromArray(array(
    'fill' => array(
        'type' => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => $bgColorTotal),
    )
));

// Apply vertical alignment, bold font, and border to all cells from column A to the final column
$finalColumnIndex = PHPExcel_Cell::stringFromColumnIndex($columnIndex + 2);
$objPHPExcel->getActiveSheet()->getStyle('A' . ($i + 3) . ':' . $finalColumnIndex . ($i + 3))->applyFromArray(array(
    'alignment' => array(
