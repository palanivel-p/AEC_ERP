...
<head>
    ...
    <title>CustomerWise Sale Report</title>
...
<div id="main-wrapper">
    ...
    $header_name ="CustomerWise Sale Report";
                ...
                ORDER BY DATE_FORMAT(sale_date, "%Y-%m") ASC';
                   ...
                   <thead>
                       <tr>
                        ...
                        $output .= $monthhead;
                        $output .= '<th rowspan="2">Sum of Qty</th>
                   ...
                   <tbody>';
                        ...
                        $sql2 = 'SELECT qty, unit_cost, p.product_name, p.product_id, c.customer_id, c.customer_name, 
                        DATE_FORMAT(sd.sale_date, "%M %Y") sale_date, sd.qtydata 
                 FROM product p 
                 INNER JOIN (SELECT sale_id, product_id, unit_cost, qty, (qty * unit_cost) qtydata, sale_date 
                 ...
                 LIMIT ' . $start . ', ' . $records_per_page;
                        ...
                        if (mysqli_num_rows($result2) > 0) {
                            ...
                            <td>' . $row2['product_name'] . '</td>';
                                ...
                                $averagerate = $monthcount > 0 ? $averageratetotal / $monthcount : 0;
                                $output .= $bobycontent;
                                $output .= '<td>' . $sumofqty . '</td>
                            <td>' . $averagerate . '</td>
                            <td>' . $totalamount . '</td>
                           </tr>';
                                $sn++;
                        ...
                        <div class="col-12 pl-3" style="display: flex; justify-content: center;">
                            <nav>
                                <ul class="pagination pagination-gutter pagination-primary pagination-sm no-bg">
                                    ...
                                    $sql_count = 'SELECT COUNT(DISTINCT p.product_id, c.customer_id, DATE_FORMAT(sd.sale_date, "%M %Y")) AS total 
                                  FROM product p 
                                  ...
                                  INNER JOIN category ct ON ct.category_id = p.sub_category';
                                    ...
                                    if ($page > 1) {
                                        echo '<li class="page-item page-indicator"><a class="page-link" href="?page_no=' . ($page - 1) . '"><i class="fa fa-angle-left"></i></a></li>';
                                    } else {
                                        echo '<li class="page-item page-indicator disabled"><a class="page-link" href="javascript:void(0);"><i class="fa fa-angle-left"></i></a></li>';
                                    }
                                    ...
                                    // Display next button
                                    if ($page < $total_pages) {
                                        echo '<li class="page-item page-indicator"><a class="page-link" href="?page_no=' . ($page + 1) . '"><i class="fa fa-angle-right"></i></a></li>';
                                    } else {
                                        echo '<li class="page-item page-indicator disabled"><a class="page-link" href="javascript:void(0);"><i class="fa fa-angle-right"></i></a></li>';
                                    }
                                    ?>
                                </ul>
                            </nav>
...
<script>
    ...
    $(document).on("click", ".excel_download", function () {
        window.location.href = "demo.php?fdate=<?php echo $fdate1?>&ldate=<?php echo $ldate1?>";
    });
</script>
