...
$objPHPExcel->getActiveSheet()->setCellValue('G2', 'Customers');
...
if (mysqli_num_rows($result) > 0) {
    $sNo = $i;
    $overallTotalAmount = 0; // Initialize overall total amount
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Sum up balance amounts to calculate overall total amount
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('G' . ($i + 2), ($customer_name));
...
$objPHPExcel->getActiveSheet()->setCellValue('F1',"Sale Outstanding Report");
...
$objWriter->save($_SERVER["DOCUMENT_ROOT"].'/saleOutstanding_report.csv');
sleep(1);

header('Content-type: application/octet-stream');
header("Content-Disposition: attachment; filename=saleOutstanding_report.csv");
readfile($_SERVER["DOCUMENT_ROOT"].'/saleOutstanding_report.csv');

@unlink($_SERVER["DOCUMENT_ROOT"].'/saleOutstanding_report.csv');
