...
$sql = "SELECT * FROM sale ORDER BY customer ASC";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B' . ($i + 2), ($sale_date));
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('K' . ($i + 2), ($remark));
        // After the loop, set the overall totals in the Excel sheet
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A' . ($i + 2) . ':K' . ($i + 2))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
    }
}
...
$objPHPExcel->getActiveSheet()->setCellValue('K' . $overallRowIndex, ''); // Assuming remarks for overall total is empty
