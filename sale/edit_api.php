...
//        if (mysqli_num_rows($resValidate) > 0 || ($adjustment_id == $old_pa_id)) {
        ...
        $sqlUpdate = "UPDATE `sale` SET `sale_id`='$sale_id',`sale_date`='$sale_date',`notes`='$notes',`customer` = '$customer_id',`payment_terms`='$payment_terms',
                        `discount` = '$discount',`tds`='$tds',`grand_total` = '$grand_total',`payment_status`='$payment_status',`status` = '$status',`due_date` = '$d_date',`e_way`='$e_way',`po_no` = '$po_no',`po_date`='$po_date',`total_tax`='$totaltax' WHERE `sale_id`='$sale_id'";
        mysqli_query($conn, $sqlUpdate);

        $sqlDelete = "DELETE FROM sale_details WHERE sale_id = '$sale_id'";
...
//            $discount = str_replace('%', '', $discount);
            ...
            $sqlUpdates = "UPDATE sale_details SET sale_details_id = '$sale_details_id',sale_id='$sale_id',customer='$customer_id',sale_date='$sale_date' WHERE id ='$ID'";
