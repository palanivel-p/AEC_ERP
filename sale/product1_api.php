...
//        $sqlValidate = "SELECT * FROM `product` WHERE product_name LIKE '%$product_id%' OR product_varient LIKE '%$product_id%'";
        $sqlValidate = "SELECT DISTINCT product_name, product_varient, product_id, product_price, stock_qty, order_tax, product_unit, brand_type FROM `product` WHERE product_name LIKE '%$product_id%' OR product_varient LIKE '%$product_id%'";
        ...
        if (mysqli_num_rows($resValidate) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($resValidate)) {
                ...
                $product_nameArr[] =  $row['product_name']. ($row['product_varient'] != ''?' - '.$row['product_varient']:'');
                ...
                $product_varientArr[] =  $row['product_varient'];
            ...
            $json_array['brand_type'] =  $brand_typeArr;
