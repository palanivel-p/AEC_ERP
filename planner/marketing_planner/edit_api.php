...
//    $sqlValidateCookie="SELECT * FROM `user_details` WHERE panel_api_key='$api_key'";
    ...
    if(mysqli_num_rows($resValidateCookie) > 0) {
        ...
        if (mysqli_num_rows($resValidate) > 0 || ($market_id==$old_pa_id))  {
            ...
            $sqlUpdate = "UPDATE `marketing` SET `marketing_id`='$market_id',`next_date`='$plan_date',`customer_name`='$customer_name',`meet_person`='$meet',`mobile`='$mobile',`communication`='$communication',`notes` = '$notes' WHERE `marketing_id`='$old_pa_id'";
            ...
            $info = urlencode("Market-Planner Edited");
