...
//    $sqlValidateCookie="SELECT * FROM `user_details` WHERE panel_api_key='$api_key'";
    ...
    if(mysqli_num_rows($resValidateCookie) > 0) {
        ...
        if (mysqli_num_rows($resValidate) > 0 || ($service_id==$old_pa_id))  {
            ...
            $sqlUpdate = "UPDATE `service` SET `service_id`='$service_id',`next_follow`='$next_date',`visit_date`='$plan_date',`customer_name`='$customer_name',`meet_whom`='$meet',`mobile`='$mobile',`communication`='$communication',`notes` = '$notes' WHERE `service_id`='$old_pa_id'";
            ...
            $info = urlencode("Service-Call Tracking Edited");
