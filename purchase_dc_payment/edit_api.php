...
//    $sqlValidateCookie="SELECT * FROM `user_details` WHERE panel_api_key='$api_key'";
    ...
    if(mysqli_num_rows($resValidateCookie) > 0) {
        ...
        if (mysqli_num_rows($resValidate) > 0)  {
            ...
            $sqlUpdate = "UPDATE `purchase_dc` SET `purchase_id`='$purchase_id',`repayment_date`='$repayment_date',`bank_name` = '$bank_name',`pay_made`='$pay_made',`repayment_mode`='$repayment_mode',`notes`='$notes',`ref_no_c`='$ref_no_c',`ref_no`='$reference_no',tds = '$tds' WHERE `repayment_id`='$repayment_id'";
            mysqli_query($conn, $sqlUpdate);

            $sqlUpdateBank = "UPDATE `bank_details` SET `ref_id`='$repayment_id',`bank_name`='$bank_name',`payment_date`='$repayment_date',`pay_from`='Purchase_dc_payment',`customer_name`='$supplier_name',`pay_mode`='$repayment_mode',`amount`='$pay_made',`ref_no`='$reference_no',`type` = 'Debit' WHERE `ref_id`='$repayment_id'";
