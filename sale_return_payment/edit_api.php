...
//    $sqlValidateCookie="SELECT * FROM `user_details` WHERE panel_api_key='$api_key'";
    ...
    if(mysqli_num_rows($resValidateCookie) > 0) {
        ...
        $sqlValidate = "SELECT * FROM `sale_return_payment` WHERE repayment_id='$repayment_id'";
        $resValidate = mysqli_query($conn, $sqlValidate);
        if (mysqli_num_rows($resValidate) > 0)  {

            $sqlUpdate = "UPDATE `sale_return_payment` SET `sale_id`='$purchase_id',`repayment_date`='$repayment_date',`bank_name` = '$bank_name',`pay_made`='$pay_made',`repayment_mode`='$repayment_mode',`notes`='$notes',`ref_no_c`='$ref_no_c',`ref_no`='$reference_no',tds = '$tds' WHERE `repayment_id`='$repayment_id'";
            mysqli_query($conn, $sqlUpdate);
            $sqlUpdateBank = "UPDATE `bank_details` SET `ref_id`='$repayment_id',`bank_name`='$bank_name',`payment_date`='$repayment_date',`pay_from`='Sale_return_payment',`customer_name`='$customer_name',`pay_mode`='$repayment_mode',`amount`='$pay_made',`ref_no`='$reference_no',`type` = 'Debit' WHERE `ref_id`='$repayment_id'";
            mysqli_query($conn, $sqlUpdateBank);

            $role=$_COOKIE['role'];
            $staff_id=$_COOKIE['user_id'];
            $staff_name=$_COOKIE['user_name'];
            $info = urlencode("Sale_Return_Payment Edited");
