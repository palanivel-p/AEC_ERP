...
error_reporting(0);
$page= $_GET['page_no'];
//$sale_return_payment_id= $_GET['sale_return_payment_id'];
...
//if($sale_return_payment_id != ""){
////    $sale_return_payment_idSql= " AND sale_return_payment_id = '".$sale_return_payment_id."'";
//    $sale_return_payment_idSql = " AND sale_return_payment_id LIKE '%" . $sale_return_payment_id . "%'";
//
//}
//else{
//    $sale_return_payment_idSql ="";
//}

if($e_category != ""){
    $e_categorySql= " AND sale_return_payment_type = '".$e_category."'";
//    $e_categorySql = " AND sale_return_payment_type LIKE '%" . $e_category . "%'";
...
<head>
    ...
    <title>Sale Return Payment</title>
...
<div id="main-wrapper">
    ...
    <?php
    $header_name ="Sale Return Payment";
    ...
    <div class="content-body">
        <div class="page-titles" style="margin-left: 21px;">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="javascript:void(0)">Sale Return Payment</a></li>
            </ol>
        </div>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Sale Return Payment List</h4>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-responsive-md" style="text-align: center;">
                            ...
                            <?php
                            // $sql = "SELECT * FROM sale_return_payment ORDER BY id  LIMIT $start,10";

//                            if($e_category == "" && $s_name== "" && $pay_mode == "" && $ref_no=="" && $f_date =="" && $t_date =="") {
                                $sql = "SELECT * FROM sale_return_payment  ORDER BY id DESC LIMIT $start,10";
//                            }
//                            else
//                            {
//                                $sql = "SELECT * FROM sale_return_payment WHERE sale_return_payment_date  BETWEEN '$from_date' AND '$to_date' $e_categorySql$s_nameSql$pay_modeSql$ref_noSql$addedBy ORDER BY id DESC LIMIT $start,10";
//                            }
                            ...
                            $sqlsale_return = "SELECT * FROM `sale_return` WHERE `sale_id`='$sale_id'";
                            $ressale_return = mysqli_query($conn, $sqlsale_return);
                            $rowsale_return = mysqli_fetch_assoc($ressale_return);
                            $grand_total =  $rowsale_return['grand_total'];
                            $customer_id =  $rowsale_return['customer'];
                            ...
                            $sqlamount="SELECT SUM(pay_made) AS pay_made  FROM sale_return_payment WHERE sale_id='$sale_id'";
...
<!--                                    <div class="dropdown">-->
<!--                                        <a href="--><?php //echo $website?><!--/Ledgers/sale_return_payment/show_file.php?sale_return_payment_id=--><?php //echo $row['sale_return_payment_id']?><!--"-->
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" data-toggle="modal" data-target="#repayment_list" style="cursor: pointer" onclick="repayment('<?php echo $row['repayment_id'];?>','<?php echo $grand_total?>','<?php echo $balance_amount?>','<?php echo $totalAmount ?>')">Edit</a>
                                <?php
                                if($_COOKIE['role'] == 'Super Admin') {
                                    ?>
...
//                                if($e_category == "" && $s_name== "" && $pay_mode == "" && $ref_no=="" && $f_date =="" && $t_date =="") {
//                                        $sqls = "SELECT COUNT(id) as count FROM sale_return_payment WHERE id>0 $addedBy";
                                    $sqls = "SELECT COUNT(id) as count FROM sale_return_payment ";

//                                }
//                                else
//                                {
//                                    $sqls = "SELECT COUNT(id) as count FROM sale_return_payment WHERE sale_return_payment_date  BETWEEN '$from_date' AND '$to_date'$e_categorySql$s_nameSql$pay_modeSql$ref_noSql$addedBy";
//                                        $sqls = "SELECT COUNT(id) as count FROM sale_return_payment";
//                                }
                ...
                <div class="basic-form" style="color: black;">
                    <form id="filter_form" autocomplete="off">
                        <div class="form-row">
                            ...
                            <!--                                <div class="form-group col-md-12">-->
                            <!--                                    <label>sale_return_payment Id </label>-->
                            <!--                                    <input type="text"  class="form-control" placeholder="sale_return_payment Id" id="sale_return_payment_id" name="sale_return_payment_id" style="border-color: #181f5a;color: black">-->
                            <!--                                </div>-->
                            <div class="form-group col-md-12">
                                <label>sale_return_payment Category </label>
                                <!--                                    <label>Category Type</label>-->
                                <select  class="form-control" id="e_category" name="e_category" style="border-color: #181f5a;color: black">
                                    <option value="">All</option>
                                    <?php
                                    $sqlDevice = "SELECT * FROM `sale_return_payment_category`";
                                ...
                                <!--                                    <input type="text"  class="form-control" placeholder="sale_return_payment Category" id="e_category" name="e_category" style="border-color: #181f5a;color: black">-->
                            </div>
                            <div class="form-group col-md-12">
...
<script>
            ...
            .then((value) => {
                if(value.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        type: "POST",
                        url: "delete_api.php",
                        data: 'sale_return_payment_id='+data,
                        dataType: "json",
                        success: function(res){
                            if(res.status=='success')
                            {
    ...
    $( document ).ready(function() {
        ...
        $("#sale_return_payment_name").trigger("change");
        $("#pay_mode").trigger("change");
        $("#s_name").trigger("change");

        $("#sale_return_payment_type").change(function(){
            var sale_return_payment_type = this.value;
            // alert(this.value);
            $("#sale_return_payment_name option").remove();
            if(sale_return_payment_type != ''){
                sale_return_payment_type_fun(sale_return_payment_type);
            }
        });
    });

    $(document).on("click", ".excel_download", function () {
        window.location.href = "excel_download.php?&sale_return_payment_id=<?php echo $sale_return_payment_id?>&e_category=<?php echo $e_category?>&s_name=<?php echo $s_name?>&pay_mode=<?php echo $pay_mode?>&ref_no = <?php echo $ref_no?>&f_date=<?php echo $from_date?>&t_date=<?php echo $to_date?>";
    });
    $(document).on("click", ".pdf_download", function () {
        window.location.href = "pdf_download.php?&sale_return_payment_id=<?php echo $sale_return_payment_id?>&e_category=<?php echo $e_category?>&s_name=<?php echo $s_name?>&pay_mode=<?php echo $pay_mode?>&ref_no = <?php echo $ref_no?>&f_date=<?php echo $from_date?>&t_date=<?php echo $to_date?>";
    });
        ...
        if(bankname != ''){
            $.ajax({
                ...
                success: function(res){
                    ...
                    //         document.getElementById('sale_return_payment_name').appendChild(opt);
                    //     }
                    //     if(category_sub != undefined){
                    //         $("#sale_return_payment_name").val(sale_return_payment_name);
