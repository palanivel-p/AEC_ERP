...
//}
if($vehicle_no != ""){
    $vehicle_noSql= " AND vehicle_no = '".$vehicle_no."'";
}
else{
    $vehicle_noSql ="";
}
                    ...
                    <div class="table-responsive">
                        <table class="table table-responsive-md" style="text-align: center;">
                            ...
                            <tr>
                                ...
                                <th><strong>other_charges</strong></th>
                            ...
                            <?php
//                             $sql = "SELECT * FROM purchase_shipment ORDER BY id  LIMIT $start,10";

                            if($vehicle_no == "" && $sh_id == "" && $f_date =="" && $t_date =="") {
                                $sql = "SELECT * FROM purchase_shipment  ORDER BY id  LIMIT $start,10";
                            }
                            else {
                                  $sql = "SELECT * FROM purchase_shipment WHERE ps_date  BETWEEN '$from_date' AND '$to_date' $sh_idSql$vehicle_noSql ORDER BY id  LIMIT $start,10";
                            }
                            ...
                            $sqlamount="SELECT SUM(pay_made) AS pay_made  FROM purchase_dc WHERE shipping_id='$shipment_id'";
...
<!--                                <td> --><?php //echo $row['bl_no']?><!-- </td>-->
                                ...
                                <td class="text-center">
                                    <div class="dropdown">
                                        <a href="<?php echo $website?>/purchase_shipment/show_file.php?shipping_id=<?php echo $shipment_id?>"
                                           class="btn btn-primary shadow btn-xs sharp me-1"><i class="fa fa-eye"></i></a>
                                    </div>
                                </td>
<!--                                <td> <a href="invoice.php?purchase_id=--><?php //echo $row['purchase_id'] ?><!--"><span class="badge badge-pill badge-primary">Download</span></a></td>-->
                                <?php
                                if($_COOKIE['role'] == 'Super Admin'|| $_COOKIE['role'] == 'Admin') {
                                ?>
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item" data-toggle="modal" data-target="#repayment_list" style="cursor: pointer" onclick="repayment('<?php echo $row['purchase_id']?>','<?php echo $row['shipping_id']?>','<?php echo $row['shipping_amount']?>','<?php echo $totalAmount?>','<?php echo $balance_amount?>')">Create Payment</a>
...
//                                    $sql = 'SELECT COUNT(id) as count FROM supplier ';
                                    $sql = "SELECT COUNT(id) as count FROM purchase_shipment ";
