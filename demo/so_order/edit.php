...
$so_id= $_GET['so_id'];
$sqlso = "SELECT * FROM so WHERE so_id ='$so_id'";
$resso = mysqli_query($conn, $sqlso);
$rowso = mysqli_fetch_assoc($resso);
$so_date =  $rowso['so_date'];
$due_date =  $rowso['due_date'];
$supplier =  $rowso['supplier'];
$currency_id =  $rowso['currency_id'];
$payment_terms =  $rowso['payment_terms'];
$discountss =  $rowso['discount'];
$discounts=  str_replace("%","",$discountss);
$grand_total =  $rowso['grand_total'];
$notes=$rowso['notes'];
$total_tax=$rowso['total_tax'];
...
<script>
    function navigateToPage() {
        window.location.href = "https://erp.aecindia.net/so/";
    }
    //due date
        ...
        if($("#staff_form").valid()==true) {
            ...
            for (let i = 0; i < tableRows.length; i++) {
                ...
                const rowData = {
                    ...
                    reasonType: row.cells[7].textContent,
                    ...
                    if (res.status == 'success') {
                        Swal.fire(
                            ...
                            .then((value) => {
                                ...
                                window.location.href = "https://erp.aecindia.net/so/all_so.php";
                            });
