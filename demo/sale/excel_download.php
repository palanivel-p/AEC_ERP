...
//                                $sql = "SELECT * FROM purchase  ORDER BY id  LIMIT $start,10";
    $sql = "SELECT * FROM sale WHERE sale_date  BETWEEN '$from_date' AND '$to_date' ORDER BY id DESC";
}
else {
    $sql = "SELECT * FROM sale  WHERE sale_date  BETWEEN '$from_date' AND '$to_date' $pur_idSql$py_stsSql ORDER BY id DESC";
}
...
if (mysqli_num_rows($result)>0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C' . ($i + 2) , ($customer));
        $objPHPExcel->getActiveSheet()->setCellValue('D' . ($i + 2) ,$grand_total);
        $objPHPExcel->getActiveSheet()->setCellValue('E' . ($i + 2) ,$py_status);
...
ob_clean();
$objPHPExcel->getActiveSheet()->setCellValue('A1'," Sales List");
//$objPHPExcel->getActiveSheet()->setCellValue('A2');
...
$objWriter->save($_SERVER["DOCUMENT_ROOT"].'/Sales.csv');
sleep(1);

header('Content-type: application/octet-stream');
header("Content-Disposition: attachment; filename=Sales.csv");
readfile($_SERVER["DOCUMENT_ROOT"].'/Sales.csv');

@unlink($_SERVER["DOCUMENT_ROOT"].'/Sales.csv');
