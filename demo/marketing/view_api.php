...
//    $sqlValidateCookie="SELECT * FROM `user_details` WHERE panel_api_key='$api_key'";
    ...
    {
        $sqlData="SELECT * FROM `market` WHERE market_id='$market_id'";
        ...
        if(mysqli_num_rows($resData) > 0)
        {
            ...
            $json_array['market_id'] = $row['market_id'];
            $json_array['visit_date'] = $row['visit_date'];
            $json_array['customer_name'] = $row['customer_name'];
            $json_array['meet'] = $row['meet_whom'];
            $json_array['sample'] = $row['sample_given'];
            $json_array['discuss_about'] = $row['discuss_about'];
            $json_array['material_name'] = $row['material_name'];
