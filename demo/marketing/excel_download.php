...
if($product_id == "") {
    $sql = "SELECT * FROM market WHERE visit_date  BETWEEN '$from_date' AND '$to_date'";
}
{
    $sql = "SELECT * FROM market WHERE visit_date  BETWEEN '$from_date' AND '$to_date' $product_idSql";
}
...
if (mysqli_num_rows($result)>0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C' . ($i + 2) , ($customer_id));
