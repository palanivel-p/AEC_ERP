...
if(isset($_POST['id']) && isset($_POST['type'])&& isset($_POST['date'])&& isset($_POST['hours'])) {
    ...
    if($hours!='00'){
        ...
        $dateTimeFormat = '0000-00-00 00:00:00';
        $present='A';
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if($type=="logIn") {
            ...
            $sqlInsert = "insert into `attendance`(`emp_id`,`date_time`,`login_lat`,`login_lng`,`login`,`present_status`,`remarks`) values ('$emp_id','$date','$lat','$lng','$dateTimeFormat','A','HR')";
...
// Construct a human-readable duration string in HH:MM:SS format
                ...
                $sqlInsert = "UPDATE `attendance` SET `logout`='$dateTimeFormat',`login`='$indateTimeFormat',`present_status`='$present',`duration`='$formattedDiff',`logout_lat`='$lat',`logout_lng`='$lng',`remarks`='HR' WHERE id='$lid' order by id desc limit 1";
                mysqli_query($conn, $sqlInsert);
...
//                mysqli_query($conn, $sqlUpdates);
                ...
                $eid=$_COOKIE['emp_id'];
                $info = urlencode("Logout manually successfully");
                $eid = urlencode($eid); // Assuming $id is a variable with the emp_id value
