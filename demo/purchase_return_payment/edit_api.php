...
//    $sqlValidateCookie="SELECT * FROM `user_details` WHERE panel_api_key='$api_key'";
    ...
    if(mysqli_num_rows($resValidateCookie) > 0) {
        ...
        if (mysqli_num_rows($resValidate) > 0)  {
            ...
            $sqlUpdate = "UPDATE `purchase_return_payment` SET `purchase_id`='$purchase_id',`repayment_date`='$repayment_date',`bank_name` = '$bank_name',`pay_made`='$pay_made',`repayment_mode`='$repayment_mode',`notes`='$notes',`ref_no_c`='$ref_no_c',`ref_no`='$ref_no' WHERE `repayment_id`='$repayment_id'";
