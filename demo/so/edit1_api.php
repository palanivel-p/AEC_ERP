...
//        if (mysqli_num_rows($resValidate) > 0 || ($adjustment_id == $old_pa_id)) {
        ...
        $sqlUpdate = "UPDATE `so` SET `so_id`='$so_id',`so_date`='$so_date',`notes`='$notes', `discount` = '$discount',`grand_total` = '$grand_total',`total_tax`='$totaltax' WHERE `so_id`='$so_id'";
        mysqli_query($conn, $sqlUpdate);

        $sqlDelete = "DELETE FROM so_details WHERE so_id = '$so_id'";
