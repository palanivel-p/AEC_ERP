...
<head>
    ...
    <title>All sale Order profile</title>
...
//                                $sql = "SELECT * FROM purchase  ORDER BY id  LIMIT $start,10";
                                $sql = "SELECT * FROM so WHERE so_date  BETWEEN '$from_date' AND '$to_date' ORDER BY id DESC LIMIT $start, 10";

                            }
                            else {
                                $sql = "SELECT * FROM so WHERE so_date  BETWEEN '$from_date' AND '$to_date' $pur_idSql ORDER BY id DESC LIMIT $start,10";
                            }
...
//                            $sale_id =  $row['sale_id'];
                            ...
                            $sqlSo = "SELECT * FROM `sale` WHERE `so_id`='$so_id'";
                            $resSo = mysqli_query($conn, $sqlSo);

                            $sale_ids = array(); // Array to store purchase IDs
                            // Loop through the result set to fetch all purchase IDs
                            while ($rowSo = mysqli_fetch_assoc($resSo)) {
                                $sale_ids[] = $rowSo['sale_id']; // Store each purchase ID in the array
                            }
                            // Join purchase IDs with comma separator
                            $sale_id = implode(',', $sale_ids);
                            ...
                            $sqlIntend = "SELECT * FROM `sale` WHERE `so_id`='$so_id'";
                            $resIntend = mysqli_query($conn, $sqlIntend);
                            $sale_ids = array(); // Array to store purchase IDs
                            // Loop through the result set to fetch all purchase IDs
                            while ($rowIntend = mysqli_fetch_assoc($resIntend)) {
                                $sale_ids[] = $rowIntend['sale_id']; // Store each purchase ID in the array
                            }
                            // Join purchase IDs with comma separator
                            $sale_id = implode(',', $sale_ids);
                            if($sale_id != ''){
                ...
                <a href="<?php echo $website; ?>/so/so_details.php?sale_id=<?php echo $sale_id?>" style="color: white;"><?php echo $sale_id?></a>
            </span>
                                            &nbsp;
                                        <?php endforeach; ?>
                                    <?php else: ?>
...
<!--                                <td> <a href="#"><span class="badge badge-pill badge-primary">--><?php //echo $sale_id?><!--</span></a></td>-->
                                ...
                                <td class="text-center">
                                    <div class="dropdown">
                                        <a href="<?php echo $website?>/so/show_file.php?so_id=<?php echo $row['so_id']?>"
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <a class="dropdown-item" href="https://erp.aecindia.net/so/edit.php?so_id=<?php echo $so_id?>" style="cursor: pointer" >Edit</a>
                                            <a class="dropdown-item" href="https://erp.aecindia.net/so_raise/index.php?so_id=<?php echo $so_id?>" style="cursor: pointer" >Create Sale</a>
                                            <?php
                                            if($_COOKIE['role'] == 'Super Admin') {
                                            ?>
                                            <a class="dropdown-item" style="cursor: pointer" onclick="delete_model('<?php echo $so_id;?>')">Delete</a>
                                                <?php
                                            }
                                            ?>
                                        </div>
                        ...
                        <div class="col-12 pl-3" style="display: flex;justify-content: center;">
                            <nav>
                                <ul class="pagination pagination-gutter pagination-primary pagination-sm no-bg">
                                    ...
                                    if($pur_id == "" ) {
//                                $sql = "SELECT * FROM purchase  ORDER BY id  LIMIT $start,10";
                                        $sql = "SELECT COUNT(id) as count FROM so WHERE so_date  BETWEEN '$from_date' AND '$to_date'$addedBy ORDER BY id DESC";
                                    }
                                    else {
                                        $sql = "SELECT COUNT(id) as count FROM so WHERE so_date  BETWEEN '$from_date' AND '$to_date' $pur_idSql$addedBy ORDER BY id DESC";
                                    }
                        ...
                        <form id="filter_form" autocomplete="off">
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-12">
                                    <label>SO Id </label>
                                    <input type="text"  class="form-control" placeholder="SO Id" id="pur_id" name="pur_id" style="border-color: #181f5a;color: black">
                                </div>
                                <div class="form-group col-md-12">
...
<script>
    ...
    $('#email_btn').click(function () {
        ...
        if($("#email_form").valid()==true) {
            ...
            $.ajax({
                ...
                type: "POST",
                url: "https://erp.aecindia.net/sale_email/index.php",
    ...
    function pdf(sale_id) {
        window.location.href= 'https://erp.aecindia.net/sale/invoice.php?sale_id='+sale_id;
    }
</script>
