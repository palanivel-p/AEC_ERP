    ...
    if (mysqli_num_rows($resValidateCookie) > 0) {
        ...
        if($service_type == 'Furnace Lining') {
            if($service_add ==1){
                ...
                $sqlUpdate_furnace = "UPDATE furnace SET service_type = '$service_type',service_id = '$service_id',visit_date = '$visit_date',capacity = '$capacity',company_name = '$company_name',furnace_no = '$furnace_no',material_type = '$material_type',gld = '$gld',fork_height = '$fork_height',sharpness = '$sharpness',tapping_temperature = '$tapping_temperature',power = '$power',panel_capacity = '$panel_capacity',product_name = '$product_name',grey = '$grey',batch_no='$batch_no',former='$former',gld_dimension = '$gld_dimension',remark = '$remark',thickness_bottom='$thickness_bottom',thickness_side='$thickness_side',total_weight='$total_weight' WHERE furnace_id ='$furnace_id'";
            ...
            $sqlInsert = "INSERT INTO `furnace`(`furnace_id`, `service_type`,`service_id`,`visit_date`,`capacity`,`company_name`,`furnace_no`,`material_type`,`gld`,`fork_height`,`sharpness`,`tapping_temperature`,`power`,`panel_capacity`,`product_name`,`grey`,`batch_no`,`former`,`gld_dimension`,`remark`,`thickness_bottom`,`thickness_side`,`total_weight`)
                                           VALUES ('','$service_type','$service_id','$visit_date','$capacity','$company_name','$furnace_no','$material_type','$gld','$fork_height','$sharpness','$tapping_temperature','$power','$panel_capacity','$product_name','$grey','$batch_no','$former','$gld_dimension','$remark','$thickness_bottom','$thickness_side','$total_weight')";
