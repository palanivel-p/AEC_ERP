...
//                                                        if($product_id == "") {
                            //                                $sql = "SELECT * FROM market WHERE visit_date  BETWEEN '$from_date' AND '$to_date'$addedBy ORDER BY id  LIMIT $start,10";
                                                                $sql = "SELECT * FROM target  ORDER BY id DESC LIMIT $start,10";
//                                                        }
//                                                        else{
...
//                                                        }
                            ...
                            $sqlVisit = "SELECT COUNT(*) AS visit_count FROM market WHERE MONTH(visit_date) = '$month_number'";
                            ...
                            $sqlCommitment = "SELECT SUM(commitment_value) AS sum_commitment FROM market WHERE MONTH(visit_date) = '$month_number'";
...
//                            }
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-primary mb-2" data-toggle="modal" data-target="#view_modal" onclick="status_fun('<?php echo $row['target_id']; ?>')" style="margin-left: 20px;">
                                        VIEW
                                    </button></td>
<!--                                <td> <a href="service_view.php?service_id=--><?php //echo $row['service_profile_id'] ?><!--"><span class="badge badge-pill badge-success">View</span></a></td>-->
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item" data-toggle="modal" data-target="#view_modal"
                                               style="cursor: pointer" onclick="status_fun('<?php echo $row['target_id']; ?>','<?php echo $month_number ?>')">Status</a>
                                            <a class="dropdown-item" data-toggle="modal" data-target="#career_list" style="cursor: pointer" onclick="editTitle('<?php echo $row['target_id'];?>')">Edit</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="tit">Expense</h5>
...
<!--                                        <select data-search="true" class="form-control js-example-disabled-results tail-select w-full" id="product_name" name="product_name" style="border-color: #181f5a;color: black" onchange="supplierfun(this.value)">-->
...
//                                                    ?>
<!--                                                    <option-->
<!--                                                            value='--><?php //echo $rowSupplier['product_id']; ?><!--'>--><?php //echo strtoupper($rowSupplier['product_name']); ?><!--</option>-->
<!--                                                    --><?php
//                                                }
//                                            }
//                                            ?>
...
<!--                                        <label>Qty *</label>-->
<!--                                        <input type="number" class="form-control" id="qty" name="qty" placeholder="Qty" style="border-color: #181f5a;color: black">-->
<!--                                    </div>-->
                    ...
                    <div class="basic-form" style="color: black;">
                        <form id="view_form" autocomplete="off">
                            <div class="form-row">
                                ...
                                $months = $_GET['months'];
                                if($plCounts > 0) {
                                    $sqlTarget = "SELECT * FROM target_detail WHERE target_id='$pusrchase_sts'";

                                    $resultTarget = mysqli_query($conn, $sqlTarget);
                                    if (mysqli_num_rows($resultTarget)>0) {
                                        $plCount_id = 0;
                                        while($rowTarget = mysqli_fetch_assoc($resultTarget)) {
                                            $plCount_id++;

                                            $product_id = $rowTarget['product_id'];
                                            $qty = $rowTarget['qty'];
                                            ...
                                            $sqlAchievedQty = "SELECT SUM(qty) AS sum_qty FROM `sale_details` WHERE product_id = '$product_id' AND sale_date = '$months'";
...
<script>
    ...
    //Status
    function status_fun(data,month) {
        window.location.href="https://erp.aecindia.net/planner/marketing_target/?purchase_sts="+data + "&months=" + month;
    }
    //status close
    ...
    $("#expense_form").validate(
        {
            ...
            messages: {
                receipt: "*Please Enter Warehouse Receipt Id",
