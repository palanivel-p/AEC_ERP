...
<head>
    ...
    <title>Service Call Tracking</title>
...
<div id="main-wrapper">
    ...
    <?php
    $header_name ="Service Call Tracking";
    ...
    <div class="content-body">
        <div class="page-titles" style="margin-left: 21px;">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="javascript:void(0)">Service Call Tracking</a></li>
            </ol>
        </div>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Service Call Tracking </h4>
...
//                                        echo "<th style='width: 100px; white-space: nowrap;'><strong>$formatted_date ($customer_count)</strong></th>";
                                    echo "<th style='width: 100px; white-space: nowrap;'><strong>$formatted_date <span style='color: red;'>($customer_count)</span></strong></th>";
                            ...
                            if($customer_name == "" && $f_date !="" && $t_date !="") {
                                ...
                                $sql = "SELECT * FROM call_service where call_date BETWEEN '$f_date' AND '$t_date' GROUP BY customer_id  ORDER BY id DESC LIMIT $start,10";
                            }
                            else if ($customer_name != "" && $f_date !="" && $t_date !=""){
                                  $sql = "SELECT * FROM call_service where call_date BETWEEN '$f_date' AND '$t_date' AND  customer_name = '$customer_name' GROUP BY customer_id  ORDER BY id DESC LIMIT $start,10";
                            ...
                            <tr>
                                ...
                                <td style="font-weight: bold">
                                    ...
                                    echo $row_count; // Output the count
                                    ?>
                                </td>
                                <?php
                                $customer_id = $row["customer_id"];
...
//                                $t_date1 = date('Y-m-d');
                                ...
                                $sqlcall = "SELECT * FROM `call_service` where customer_id='$customer_id'";
...
//                                    while ($current_date <= $t_date1) {
//                                        $formatted_date = date('Y-m-d', strtotime($current_date)); // Formatting date as yyyy-mm-dd for comparison
//                                        // Display count for the current date
//                                        if (isset($counts[$formatted_date])) {
//                                            // If the value is 'C', add onclick attribute
//                                            if ($row['customer_name'] == 'C') {
//                                                echo "<td><strong style='color: orangered;font-size: 25px;font-weight: 900; cursor: pointer' onclick=\"repayment('$customer_name','$notes','$call_date)\">C</strong></td>";
//                                            } else {
...
//                                    }
                                    ...
                                    while ($current_date <= $t_date) {
                                        ...
                                        if (isset($counts[$formatted_date])) {
                                            echo "<td>";
                                            $sqlIntend = "SELECT * FROM `call_service` WHERE `customer_name`='$customer_name' AND call_date='$formatted_date'";
                                            $resIntend = mysqli_query($conn, $sqlIntend);
                                            while ($rowIntend = mysqli_fetch_assoc($resIntend)) {
                                                $call_id = $rowIntend['call_track_id'];

                                                $sqlCall = "SELECT * FROM `call_service` WHERE `call_service_id`='$call_id'";
                                                $resCall = mysqli_query($conn, $sqlCall);
                                                $rowCall = mysqli_fetch_assoc($resCall);
                                                $customer_name1 = $rowCall['customer_name'];
                                                $notes1 = $rowCall['notes'];
                                                $call_date1 = $rowCall['call_date'];
                                                $next_date1 = $rowCall['next_date'];

                                                echo "<strong style='color: orangered; font-size: 25px; font-weight: 900; cursor: pointer' onclick=\"repayment('$customer_name1','$notes1','$call_date1','$next_date1','$call_id')\">C</strong> ";
                                            }
                                            echo "</td>";
                                        } else {
                                            echo "<td>NA</td>";
                        ...
                        <div class="col-12 pl-3" style="display: flex;justify-content: center;">
                            <nav>
                                <ul class="pagination pagination-gutter pagination-primary pagination-sm no-bg">
                                    ...
                                    {
                                        ?>
                                        <li class="page-item page-indicator"><a class="page-link" href="?page_no=<?php echo $prevPage?>&product_id=<?php echo $customer_name ?>&f_date=<?php echo $from_date?>&t_date=<?php echo $to_date?>"><i class="fa-solid fa-less-than"></i></a></li>
                                        <?php
                                    }
...
//                                        $sql = "SELECT COUNT(id) as count FROM call_track GROUP BY customer_id";
                                    if($customer_name == "" && $f_date !="" && $t_date !="") {

                                        $sql = "SELECT COUNT(id) as count FROM call_service where call_date BETWEEN '$f_date' AND '$t_date'";
                                    }
                                    else if ($customer_name != "" && $f_date !="" && $t_date !=""){
                                        $sql = "SELECT COUNT(id) as count FROM call_service where call_date BETWEEN '$f_date' AND '$t_date' AND  customer_name = '$customer_name'";
                                    ...
                                    if (mysqli_num_rows($result)) {
                                        ...
                                        for ($i = 1; $i <= $pageFooter; $i++) {
                                            ...
                                            if($i<=($pageSql+10) && $i>$pageSql || $pageFooter<=10) {
                                                ...
                                                <li class="page-item <?php echo $active ?>"><a class="page-link"
                                                                                               href="?page_no=<?php echo $i ?>&product_id=<?php echo $customer_name ?>&f_date=<?php echo $f_date?>&t_date=<?php echo $t_date?>"><?php echo $i ?></a>
                                                </li>
                                                <?php
                                            }
                                        }
                                        ...
                                        {
                                            ?>
                                            <li class="page-item page-indicator"><a class="page-link" href="?page_no=<?php echo $nextPage ?>&product_id=<?php echo $customer_name ?>&f_date=<?php echo $f_date?>&t_date=<?php echo $t_date?>"><i class="fa-solid fa-greater-than"></i></a></li>
                                            <?php
                                        }
                                    }
                                    ?>
                            ...
                            <form id="call_form" autocomplete="off">
                                <div class="form-row">
                                    <div class="form-group col-md-12">
                                        <label>Call Date </label>
                                        <input type="date" class="form-control" id="call_date" name="call_date" readonly style="border-color: #181f5a;color: black">
                                    </div>
                                    <div class="form-group col-md-12">
                                        <label>Customer Name</label>
...
<!--                                        <input type="text" class="form-control" placeholder="Note" id="note" name="note" readonly style="border-color: #181f5a;color: black">-->
                                    </div>
                                    <div class="form-group col-md-12">
                                        <label>Next FollowUp Date*</label>
                                        <input type="date" class="form-control" id="next_dates" name="next_dates" readonly style="border-color: #181f5a;color: black">
                                    </div>
