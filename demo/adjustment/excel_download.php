...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Adjustment Id');
$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Adjustment Date');
...
if($a_id == "" && $f_reason == "") {
    $sql = "SELECT * FROM adjustment WHERE adjustment_date  BETWEEN '$from_date' AND '$to_date'ORDER BY id DESC";
}
else {
    $sql = "SELECT * FROM adjustment WHERE adjustment_date  BETWEEN '$from_date' AND '$to_date' $a_idSql$reasonSql ORDER BY id DESC";
}
...
if (mysqli_num_rows($result)>0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B' . ($i + 2) , ($adjustment_id));
        $objPHPExcel->getActiveSheet()->setCellValue('C' . ($i + 2) , ($adjustment_date));
        $objPHPExcel->getActiveSheet()->setCellValue('D' . ($i + 2) ,$notes);
        $objPHPExcel->getActiveSheet()->setCellValue('E' . ($i + 2) ,$res);
