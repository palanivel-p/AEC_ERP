...
//        if (mysqli_num_rows($resValidate) > 0 || ($adjustment_id == $old_pa_id)) {
        ...
        $sqlUpdate = "UPDATE `purchase_v1` SET `purchase_id`='$purchase_id',`purchase_date`='$purchase_date',`notes`='$notes',`supplier` = '$supplier_id',`currency_id` = '$currency',`payment_terms`='$payment_terms',
                        `discount` = '$discount',`grand_total` = '$grand_total',`payment_status`='$payment_status',`status` = '$status',`due_date` = '$d_date',`material_date`='$material',`transport` = '$transport',`total_tax`='$totaltax' WHERE `purchase_id`='$purchase_id'";
...
//            $discount = str_replace('%', '', $discount);
            ...
            $sqlInsersts = "INSERT INTO purchase_details (purchase_details_id,product_id,product_name,unit,unit_cost, stock,Stock_count ,discount,dis_symbl, tax, sub_total,qty,tax_value,discount_value,discount_type) 
                    VALUES ('','$productId','$productName','$unit', '$netUnitCost', '$stockValue','$stock_add', '$discount','$persymbl', '$tax', '$subtotal','$quantity','$tax_value','$discount_value','$symbl')";
