...
if($sale_payment_id != ""){
//    $sale_payment_idSql= " AND sale_payment_id = '".$sale_payment_id."'";
    $sale_payment_idSql = " AND sale_payment_id LIKE '%" . $sale_payment_id . "%'";
...
if($e_category != ""){
    $e_categorySql= " AND sale_payment_type = '".$e_category."'";
//    $e_categorySql = " AND sale_payment_type LIKE '%" . $e_category . "%'";
    ...
    <div class="content-body">
        <div class="page-titles" style="margin-left: 21px;">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="javascript:void(0)">Sale Payment</a></li>
            </ol>
        </div>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Sale Payment </h4>
...
<!--                                <th><strong>Due Amount</strong></th>-->
                            ...
                            <?php
                            // $sql = "SELECT * FROM sale_payment ORDER BY id  LIMIT $start,10";

//                            if($e_category == "" && $s_name== "" && $pay_mode == "" && $ref_no=="" && $f_date =="" && $t_date =="") {
                                $sql = "SELECT * FROM sale_payment  ORDER BY id DESC LIMIT $start,10";
//                            }
//                            else
//                            {
//                                $sql = "SELECT * FROM sale_payment WHERE sale_payment_date  BETWEEN '$from_date' AND '$to_date' $e_categorySql$s_nameSql$pay_modeSql$ref_noSql$addedBy ORDER BY id DESC LIMIT $start,10";
//                            }
...
<!--                                    <div class="dropdown">-->
<!--                                        <a href="--><?php //echo $website?><!--/Ledgers/sale_payment/show_file.php?sale_payment_id=--><?php //echo $row['sale_payment_id']?><!--"-->
...
//                                if($e_category == "" && $s_name== "" && $pay_mode == "" && $ref_no=="" && $f_date =="" && $t_date =="") {
//                                        $sqls = "SELECT COUNT(id) as count FROM sale_payment WHERE id>0 $addedBy";
                                    $sqls = "SELECT COUNT(id) as count FROM sale_payment ";

//                                }
//                                else
//                                {
//                                    $sqls = "SELECT COUNT(id) as count FROM sale_payment WHERE sale_payment_date  BETWEEN '$from_date' AND '$to_date'$e_categorySql$s_nameSql$pay_modeSql$ref_noSql$addedBy";
//                                        $sqls = "SELECT COUNT(id) as count FROM sale_payment";
//                                }
                ...
                <div class="basic-form" style="color: black;">
                    <form id="filter_form" autocomplete="off">
                        <div class="form-row">
                            ...
                            <!--                                <div class="form-group col-md-12">-->
                            <!--                                    <label>sale_payment Id </label>-->
                            <!--                                    <input type="text"  class="form-control" placeholder="sale_payment Id" id="sale_payment_id" name="sale_payment_id" style="border-color: #181f5a;color: black">-->
                            <!--                                </div>-->
                            <div class="form-group col-md-12">
                                <label>sale_payment Category </label>
                                <!--                                    <label>Category Type</label>-->
                                <select  class="form-control" id="e_category" name="e_category" style="border-color: #181f5a;color: black">
                                    <option value="">All</option>
                                    <?php
                                    $sqlDevice = "SELECT * FROM `sale_payment_category`";
                                ...
                                <!--                                    <input type="text"  class="form-control" placeholder="sale_payment Category" id="e_category" name="e_category" style="border-color: #181f5a;color: black">-->
                            </div>
                            <div class="form-group col-md-12">
...
<script>
    ...
    $( document ).ready(function() {
        ...
        $("#sale_payment_name").trigger("change");
        $("#pay_mode").trigger("change");
        $("#s_name").trigger("change");

        $("#sale_payment_type").change(function(){
            var sale_payment_type = this.value;
            // alert(this.value);
            $("#sale_payment_name option").remove();
            if(sale_payment_type != ''){
                sale_payment_type_fun(sale_payment_type);
            }
        });
    });

    $(document).on("click", ".excel_download", function () {
        window.location.href = "excel_download.php?&sale_payment_id=<?php echo $sale_payment_id?>&e_category=<?php echo $e_category?>&s_name=<?php echo $s_name?>&pay_mode=<?php echo $pay_mode?>&ref_no = <?php echo $ref_no?>&f_date=<?php echo $from_date?>&t_date=<?php echo $to_date?>";
    });
    $(document).on("click", ".pdf_download", function () {
        window.location.href = "pdf_download.php?&sale_payment_id=<?php echo $sale_payment_id?>&e_category=<?php echo $e_category?>&s_name=<?php echo $s_name?>&pay_mode=<?php echo $pay_mode?>&ref_no = <?php echo $ref_no?>&f_date=<?php echo $from_date?>&t_date=<?php echo $to_date?>";
    });
        ...
        if(bankname != ''){
            $.ajax({
                ...
                success: function(res){
                    ...
                    //         document.getElementById('sale_payment_name').appendChild(opt);
                    //     }
                    //     if(category_sub != undefined){
                    //         $("#sale_payment_name").val(sale_payment_name);
