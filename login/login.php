...
//      $pw_hash = sha1($salt.$password);
    ...
    if (mysqli_num_rows($resValidate) > 0) {
        ...
        setcookie("user_name", 'Super Admin', time() + (3600 * $cookie_set), "/"); // To set Login for 1 h
        setcookie("user_id", 'Super Admin', time() + (3600 * $cookie_set), "/"); // To set Login for 1 h
        ...
        echo $json_response;
            ...
            $sqlValidateS = "SELECT * FROM `user` WHERE email='$email' AND password='$password' AND access_status=1";
